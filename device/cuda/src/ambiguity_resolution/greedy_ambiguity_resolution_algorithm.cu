...
greedy_ambiguity_resolution_algorithm::operator()(
    ...
    // Per-outer-iteration batch-size cell, written by the conflict-graph
    // path's apply_graph_removals kernel. Allocated unconditionally so the
    // resolver-graph capture sees a stable address.
    ...
    while (!terminate && n_accepted > 0) {
        ...
        if (m_graph_algo != graph_algo_t::NONE) {
            ...
            //   4. Run a single Jones-Plassmann coloring round; the round
            //      selects exactly the local-priority-maximal vertices
            //      (color class 0), which are an independent set in the
            //      conflict graph by construction.
        ...
        // Baseline single-block batched removal with the prefix
        // conflict-free admission rule. Unchanged from upstream and used
        // whenever the explicit conflict-graph path is not enabled.
    ...
    // Conflict-graph mode per-iteration logs.
    if (m_graph_batch_log != nullptr) {
