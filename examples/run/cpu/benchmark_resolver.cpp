...
struct PhaseTimingMs {
    double filter_setup =
        0;  ///< Allocate vectors, filter by min_meas, build meas_ids
    double unique_meas = 0;  ///< Collect unique measurement IDs and sort them
    double inverted_index =
        0;                    ///< Build tracks_per_measurement (inverted index)
    double shared_count = 0;  ///< Count n_shared and rel_shared per track
    double initial_sort = 0;  ///< Sort sorted_ids by (rel_shared, pval)
    double eviction_loop =
        0;  ///< Iterative worst-track removal + bookkeeping updates
    double output_copy = 0;  ///< Copy accepted tracks into output container
    unsigned int n_iterations =
        0;  ///< Number of eviction iterations actually executed
    unsigned int n_shared_updates =
        0;  ///< Number of n_shared[tid] decrements in eviction
    std::size_t unique_meas_count =
        0;  ///< Size of the unique measurement universe
...
static PhaseTimingMs run_with_phase_timing(
    ...
    //     - sorted_ids find+reinsert: O(n_accepted) per updated track  ← likely
    //     hotspot
    // ------------------------------------------------------------------
    for (unsigned int iter = 0; iter < config.max_iterations; ++iter) {
        ...
        if (max_shared < config.max_shared_meas)
            break;
        ...
        for (const std::size_t mid : to_remove) {
            ...
            if (tracks.size() == 1u) {
                ...
                n_shared[tid] -= static_cast<unsigned int>(std::count(
                    meas_ids[tid].begin(), meas_ids[tid].end(), mid));
                ...
                if (it3 != sorted_ids.end()) {
                    const auto it4 = std::lower_bound(sorted_ids.begin(), it3,
                                                      tid, track_cmp);
                    if (it3 != it4) {
